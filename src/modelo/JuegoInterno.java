...
// Recoger----------------------------------------------------------------------------------------------------------------------------------------	
			...
			while(jugadoresRecorrer!=null){
				if (mesaRecorrer.getNumero().equals(jugadoresRecorrer.getNumero())){
					if(cartas.buscarRepetidos(cartas,mesaRecorrer.getIndiceCarta()))
						cartas.InsertarCarta(mesaRecorrer.getTipo(),mesaRecorrer.getNumero(),mesaRecorrer.getIndiceCarta());
					logico = true;																
				}
		...
		while(cartas!=null){		
			while(mesaRecorrer!=null){
				if (mesaRecorrer.getIndiceCarta() == cartas.getIndiceCarta()){
						guardados.InsertarCarta(mesaRecorrer.getTipo(),mesaRecorrer.getNumero(),mesaRecorrer.getIndiceCarta());
				}
				mesaRecorrer = mesaRecorrer.getProximo();	
			}
			mesaRecorrer = mesaSacar.getMazo();	
	...
	void recogerCartas(Mazo sacarMesa,Jugador jugadorPuntos,Mazo insertarAcumulado,Mazo lista,boolean identificar){
		...
		System.out.println();
		System.out.println("   " + jugadorPuntos.getNombre() + " recoge la carta: " + cartaMesa.getTipo() + " " + cartaMesa.getNumero());	
...
// Lanzar Carta------------------------------------------------------------------------------------------------------------------------
	public void lanzarCarta(Jugador jugadores,Mazo lanzaAmesa,Mazo sacaDMazoJugador,boolean identificar){
		...
		System.out.println();
		System.out.println("   " + jugadores.getNombre() + " lanza la carta: " + lanzarAmesa.getTipo() + " " + lanzarAmesa.getNumero());	
...
//AgruparSumasDeCartas
	public void RecogerCartaAgrupada(Mazo mazoJugador, Mazo mazoAgrupadorJugador) {
		...
		System.out.print("\nIngrese el tipo de la carta (de su mano): ");tipo = leer.nextLine();
		leer.nextLine();
		System.out.print("\nIngrese el numero de la carta (de su mano): ");numero = leer.nextLine();
		leer.nextLine();
					...
					if (numero.equals("AS")) {
						...
						if(auxSuma>=1 || auxSuma<=10) {
							carta = aux.BuscarCarta(aux, tipo, numero);
								if(carta != null) {
									System.out.print("\nIngrese la otra carta de tipo (de su mano): ");tip = leer.nextLine();
									//leer.nextLine();
									System.out.print("\nIngrese la otra carta de numero (de su mano): ");num = leer.nextLine();
									leer.nextLine();
									carta2 = aux3.BuscarCarta(aux3, tip, num);
									
									if (carta2 != null) {
										System.out.println("\nCuantas cartas quieres agarrar: "); cantidad = leer.nextInt();
										leer.nextLine();
										for(int i = 1; i<=cantidad;i++) {
											System.out.print("\nIngrese el tipo de la carta (de la mesa): ");t = leer.nextLine();
											//leer.nextLine();
											System.out.print("\nIngrese el numero de la carta (de la mesa): ");n = leer.nextLine();
											leer.nextLine();
	...
	public void turnoPersonaJugar(boolean salir){
		...
		do{
			...
			/*if(PuedeRecoger(juego.getMazoMesa().getMazo(),juego.getPersona().getJuego(),cartas))
				...
				opcion = 7;
				switch (opcion){
		    		case 1: if(PuedeRecoger(juego.getMazoMesa().getMazo(),juego.getPersona().getJuego(),cartas)) {
		    				...
		    				break;
					case 2: AgruparSumaDeCartas(juego.getPersona().getJuego(), juego.getMazoMesa(), juego.getPersona().getAcumulado() );
							break;
					case 3:	RecogerCartaAgrupada(juego.getPersona().getJuego(),juego.getPersona().getAcumulado());//Mazo mazoJugador, Mazo mazoAgrupadorJugador) {
							break;
					case 4: System.out.println( "No terminado....");
							break;
			    	case 5: System.out.println( "No terminado....");
			    	    	break;   	
					case 6: System.out.println( "No terminado....");						
			    	    	break;															
					case 7: lanzarCarta(juego.getPersona(),juego.getMazoMesa(),juego.getPersona().getJuego(),true);
							permitir = false;
			    	    	break;
	...
	public void turnoComputadora(){
		...
		switch (opcion){
		    case 1: if (validar) 		
					...
					break;
			case 4: lanzarCarta(juego.getComputadora(),juego.getMazoMesa(),juego.getComputadora().getJuego(),false);
			        break;	    
		}		
	}
}